...
import { useQuery, QueryClient, QueryClientProvider, Hydrate, dehydrate } from "@tanstack/react-query";
import axios from "axios";
import Image from "next/image";
import { GetStaticProps } from "next"; // ou GetServerSideProps se for SSR
...
const InstagramSection: React.FC = ({ dehydratedState }: any) => {
  ...
  return (
    <QueryClientProvider client={queryClient}>
      {/* Hidrata o estado do React Query */}
...
// Exemplo com `getStaticProps` (para SSG)
export const getStaticProps: GetStaticProps = async () => {
  const queryClient = new QueryClient();

  await queryClient.prefetchQuery(["instagram-media-home"], getGraphHome);

  return {
    props: {
      dehydratedState: dehydrate(queryClient), // Hidrata os dados pré-carregados
    },
  };
};
